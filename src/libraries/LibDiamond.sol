...
library LibDiamond {
  ...
  function enforceIsContractOwner() internal view {
    if (msg.sender != contractOwner()) {
      revert NotContractOwner(msg.sender, contractOwner());
    }
  }
  ...
  function addFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
    ...
    enforceHasContractCode(_facetAddress, "LibDiamond: addFunctions: _facetAddress has no code");
  ...
  function replaceFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
    ...
    enforceHasContractCode(_facetAddress, "LibDiamond: replaceFunctions: _facetAddress has no code");
    for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
      bytes4 selector = _functionSelectors[selectorIndex];
